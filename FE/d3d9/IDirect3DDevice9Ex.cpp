...
HRESULT m_IDirect3DDevice9Ex::CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS *pPresentationParameters, IDirect3DSwapChain9 **ppSwapChain)
{
	...
	D3D9::PopulateD3DPresentParams(&tmp);

	HRESULT hr = ProxyInterface->CreateAdditionalSwapChain(&tmp, ppSwapChain);
	...
	if (!SUCCEEDED(hr))
	{
		...
		D3D9::D3DPresentParamsCopyOnReturn(&tmp, pPresentationParameters);
		MESSAGE("OK");
	}
...
{
	return ProxyInterface->PresentEx(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, 0);
}
...
{
	return ProxyInterface->PresentEx(pSourceRect, pDestRect, hDestWindowOverride, pDirtyRegion, dwFlags | D3D9::dwPresentFlags);
}
...
HRESULT m_IDirect3DDevice9Ex::SetMaximumFrameLatency(THIS_ UINT MaxLatency)
{
	if (D3D9::MaxFrameLatency > -1) {
		MaxLatency = static_cast<UINT>(D3D9::MaxFrameLatency);
	}

	auto hr = ProxyInterface->SetMaximumFrameLatency(MaxLatency);
...
HRESULT m_IDirect3DDevice9Ex::ResetEx(THIS_ D3DPRESENT_PARAMETERS* pPresentationParameters, D3DDISPLAYMODEEX *pFullscreenDisplayMode)
{
	...
	auto hr = ProxyInterface->ResetEx(&pp, pFullscreenDisplayMode);
	if (hr == S_OK) {
